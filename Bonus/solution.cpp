...
long int find_max_column(const std::vector<std::vector<int>>& farm, int columnCount,
                         int& columnIndex) {
    auto maxColumn = std::max_element(
        farm[0].begin(), farm[0].end(), [&farm, columnCount](int column1, int column2) {
...
void harvest_a_column(std::vector<std::vector<int>>& farm, int columnIndex, int rowCount) {
    for (int i = 0; i < rowCount; i++) farm[i][columnIndex] = 0;
}

long int find_most_profitable(int rowCount, int columnCount, std::vector<std::vector<int>>& farm,
                              int numOfMoves) {
...
std::vector<std::vector<int>> get_input(int rowCount, int columnCount) {
    ...
    for (int i = 0; i < rowCount; i++)
        for (int j = 0; j < columnCount; j++) std::cin >> result[i][j];
